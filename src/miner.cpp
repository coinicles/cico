...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setCicoSchedule(cicoDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = cicoDGP.getBlockSize(nHeight);
    minGasPrice = cicoDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = cicoDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    CicoTxConverter convert(iter->GetTx(), NULL, &pblock->vtx, contractflags);

    ExtractCicoTX resultConverter;
    if(!convert.extractionCicoTransactions(resultConverter)){
    ...
    for(CicoTransaction cicoTransaction : cicoTransactions){
        txGas += cicoTransaction.gas();
        if(txGas > txGasLimit) {
            // Limit the tx gas limit by the soft limit if such a limit has been specified.
            return false;
        }

        if(bceResult.usedGas + cicoTransaction.gas() > softBlockGasLimit){
            //if this transaction's gasLimit could cause block gas limit to be exceeded, then don't add it
            return false;
        }
        if(cicoTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, cicoTransactions, hardBlockGasLimit, ::ChainActive().Tip());
...
public:
    ...
    void Update(int32_t nHeight)
    {
        ...
        if(nHeight <= nCheckpointSpan)
        {
            ...
            cicoDelegations.FilterDelegationEvents(events, *this);
            delegations_staker = cicoDelegations.DelegationsFromEvents(events);
        }
        else
        {
            ...
            {
                std::vector<DelegationEvent> events;
                cicoDelegations.FilterDelegationEvents(events, *this, cacheHeight, cpsHeight);
                cicoDelegations.UpdateDelegationsFromEvents(events, cacheDelegationsStaker);
                cacheHeight = cpsHeight;
            }

            // Update the wallet delegations
            std::vector<DelegationEvent> events;
            cicoDelegations.FilterDelegationEvents(events, *this, cacheHeight + 1);
            delegations_staker = cacheDelegationsStaker;
            cicoDelegations.UpdateDelegationsFromEvents(events, delegations_staker);
        }
...
private:
    CWallet *pwallet;
    CicoDelegation cicoDelegations;
    int32_t cacheHeight;
    ...
    void Update(interfaces::Chain::Lock& locked_chain, int32_t nHeight)
    {
        if(fLogEvents)
        {
            ...
            if(nHeight <= nCheckpointSpan)
            {
                ...
                std::vector<DelegationEvent> events;
                cicoDelegations.FilterDelegationEvents(events, *this);
                pwallet->m_my_delegations = cicoDelegations.DelegationsFromEvents(events);
            }
            else
            {
                // Update the cached delegations for the staker, older then the sync checkpoint (500 blocks)
                ...
                if(cacheHeight < cpsHeight)
                {
                    std::vector<DelegationEvent> events;
                    cicoDelegations.FilterDelegationEvents(events, *this, cacheHeight, cpsHeight);
                    cicoDelegations.UpdateDelegationsFromEvents(events, cacheMyDelegations);
                    cacheHeight = cpsHeight;
                }
                ...
                cicoDelegations.UpdateDelegationsFromEvents(events, pwallet->m_my_delegations);
            }
        }
        else
        {
            ...
            if(cacheHeight != nHeight)
            {
                ...
                for(auto item: mapAddress)
                {
                    ...
                    if(cicoDelegations.GetDelegation(address, delegation) && CicoDelegation::VerifyDelegation(address, delegation))
                    {
...
void ThreadStakeMiner(CWallet *pwallet, CConnman* connman)
{
    ...
    std::string threadName = "cicostake";
...
void StakeCicos(bool fStake, CWallet *pwallet, CConnman* connman, boost::thread_group*& stakeThread)
{
