...
#if defined(NDEBUG)
# error "Cico cannot be compiled without assertions."
#endif
...
std::unique_ptr<CicoState> globalState;
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if(tx.HasCreateOrCall()){
        ...
        uint64_t minGasPrice = cicoDGP.getMinGasPrice(::ChainActive().Tip()->nHeight + 1);
        uint64_t blockGasLimit = cicoDGP.getBlockGasLimit(::ChainActive().Tip()->nHeight + 1);
        size_t count = 0;
        ...
        CicoTxConverter converter(tx, NULL, NULL, contractflags);
        ExtractCicoTX resultConverter;
        if(!converter.extractionCicoTransactions(resultConverter)){
        ...
        std::vector<CicoTransaction> cicoTransactions = resultConverter.first;
        std::vector<EthTransactionParams> cicoETP = resultConverter.second;

        dev::u256 sumGas = dev::u256(0);
        dev::u256 gasAllTxs = dev::u256(0);
        for(CicoTransaction cicoTransaction : cicoTransactions){
            sumGas += cicoTransaction.gas() * cicoTransaction.gasPrice();
            ...
            if(txMinGasPrice != 0) {
                txMinGasPrice = std::min(txMinGasPrice, cicoTransaction.gasPrice());
            } else {
                txMinGasPrice = cicoTransaction.gasPrice();
            }
            VersionVM v = cicoTransaction.getVersion();
            if(v.format!=0)
            ...
            if(cicoTransaction.gas() < gArgs.GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
            ...
            if(cicoTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
            ...
            if(cicoTransaction.gas() > UINT32_MAX)
            ...
            if(v.rootVM!=0 && (uint64_t)cicoTransaction.gasPrice() < minGasPrice)
...
bool MemPoolAccept::Finalize(ATMPArgs& args, Workspace& ws)
{
    ...
    for (CTxMemPool::txiter it : allConflicting)
    {
        LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s CICO additional fees, %d delta bytes\n",
...
DisconnectResult CChainState::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // cico
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // cico
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit){
    ...
    CicoTransaction callTransaction(0, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());

    
    ByteCodeExec exec(block, std::vector<CicoTransaction>(1, callTransaction), blockGasLimit, pblockindex);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path cicoDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(cicoDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(cicoDir.string(), std::ios::in | std::ios::out);
...
bool ByteCodeExec::performByteCode(dev::eth::Permanence type){
    for(CicoTransaction& tx : txs){
        ...
        if(!tx.isCreation() && !globalState->addressInUse(tx.receiveAddress())){
            ...
            result.push_back(ResultExecute{execRes, CicoTransactionReceipt(dev::h256(), dev::h256(), dev::u256(), dev::eth::LogEntries()), CTransaction()});
            continue;
        }
...
bool CicoTxConverter::extractionCicoTransactions(ExtractCicoTX& cicotx){
    ...
    std::vector<CicoTransaction> resultTX;
    ...
    cicotx = std::make_pair(resultTX, resultETP);
    return true;
}

bool CicoTxConverter::receiveStack(const CScript& scriptPubKey){
    sender = false;
...
bool CicoTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
CicoTransaction CicoTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    CicoTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = CicoTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = CicoTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
...
size_t CicoTxConverter::correctedStackSize(size_t size){
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // cico
    CicoDGP cicoDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setCicoSchedule(cicoDGP.getGasSchedule(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1) ));
    uint32_t sizeBlockDGP = cicoDGP.getBlockSize(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t minGasPrice = cicoDGP.getMinGasPrice(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t blockGasLimit = cicoDGP.getBlockGasLimit(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // cico
...
///////////////////////////////////////////////////////////////////////////////////////// cico
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            CicoTxConverter convert(tx, &view, &block.vtx, contractflags);

            ExtractCicoTX resultConvertCicoTX;
            if(!convert.extractionCicoTransactions(resultConvertCicoTX)){
            ...
            if(!CheckMinGasPrice(resultConvertCicoTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertCicoTX.first, blockGasLimit, pindex->pprev);
            ...
            for(CicoTransaction& qtx : resultConvertCicoTX.first){
            ...
            if (fLogEvents && !fJustCheck)
            {
                ...
                for(size_t k = 0; k < resultConvertCicoTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{
                        ...
                        tx.GetHash(),
                        uint32_t(i),
                        resultConvertCicoTX.first[k].from(),
                        resultConvertCicoTX.first[k].to(),
                        ...
                        resultConvertCicoTX.first[k].getNVout()
                    });
                }
...
bool CChainState::ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool)
{
    ...
    LogPrint(BCLog::BENCH, "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * MILLI, nTimeReadFromDisk * MICRO);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // cico
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // cico
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // cico
            globalState->setRootUTXO(oldHashUTXORoot); // cico
...
//////////////////////////////////////////////////////////////////////////
    ...
    for (pindex = ::ChainActive().Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        ///////////////////////////////////////////////////////////////////// // cico
        uint32_t sizeBlockDGP = cicoDGP.getBlockSize(pindex->nHeight);
        dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
        updateBlockSizeParams(dgpMaxBlockSize);
        /////////////////////////////////////////////////////////////////////
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for Cico PoS we need to sync this after every block to ensure txdb is populated for
