...
bool ToHash160(const std::string& strCicoAddress, std::string& strHash160)
{
    CTxDestination cicoAddress = DecodeDestination(strCicoAddress);
    if(!IsValidDestination(cicoAddress))
        return false;
    const PKHash * keyid = boost::get<PKHash>(&cicoAddress);
    if(keyid){
...
bool ToCicoAddress(const std::string& strHash160, std::string& strCicoAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    PKHash keyid(key);
    CTxDestination cicoAddress = keyid;
    if(IsValidDestination(cicoAddress)){
        strCicoAddress = EncodeDestination(cicoAddress);
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToCicoAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToCicoAddress(tokenEvent.receiver, tokenEvent.receiver);
