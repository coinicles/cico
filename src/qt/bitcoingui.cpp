...
#include <qt/cicoversionchecker.h>
    ...
    m_network_style(networkStyle)
{
    ...
    cicoVersionChecker = new CicoVersionChecker(this);
#ifdef ENABLE_WALLET
    if(enableWallet) {
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Cico address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and cico: URIs)"));
    ...
    QRCTokenAction = new QAction(platformStyle->MultiStatesIcon(":/icons/qrctoken"), tr("&CICORC Tokens"), this);
    QRCTokenAction->setStatusTip(tr("CICORC Tokens (send, receive or add Tokens in list)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Cico addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Cico addresses"));
    ...
    openAction->setStatusTip(tr("Open a cico: URI or payment request"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Cico command-line options").arg(PACKAGE_NAME));
...
void BitcoinGUI::setClientModel(ClientModel *_clientModel)
{
    ...
    {
        // Check for updates
        if(_clientModel->getOptionsModel()->getCheckForUpdates() && cicoVersionChecker->newVersionAvailable())
        {
            QString link = QString("<a href=%1>%2</a>").arg(CICO_RELEASES, CICO_RELEASES);
            QString message(tr("New version of Cico wallet is available on the Cico source code repository: <br /> %1. <br />It is recommended to download it and update this application").arg(link));
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Cico network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
