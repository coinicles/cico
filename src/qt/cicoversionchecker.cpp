...
#define paternVersion "cico-([0-9]+\\.)?([0-9]+\\.)?([0-9]+)-"

CicoVersionChecker::CicoVersionChecker(QObject *parent) : QObject(parent)
{
    currentVersion = Version(CLIENT_VERSION_MAJOR, CLIENT_VERSION_MINOR, CLIENT_VERSION_REVISION);
}

CicoVersionChecker::~CicoVersionChecker()
{

}

bool CicoVersionChecker::newVersionAvailable()
{
    Version maxReleaseVersion = getMaxReleaseVersion();
    return maxReleaseVersion > currentVersion;
}

QList<Version> CicoVersionChecker::getVersions()
{
    QNetworkAccessManager manager;
    QNetworkReply *response = manager.get(QNetworkRequest(QUrl(CICO_RELEASES)));
    QEventLoop event;
...
Version CicoVersionChecker::getMaxReleaseVersion()
{
