...
void checkResult(bool isCreation, std::vector<CicoTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    CicoTxConverter converter(transaction, NULL);
    ExtractCicoTX cicoTx;
    BOOST_CHECK(converter.extractionCicoTransactions(cicoTx));
    std::vector<CicoTransaction> result = cicoTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(cicotxconverter_tests, TestingSetup)
