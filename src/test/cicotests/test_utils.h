...
extern std::unique_ptr<CicoState> globalState;
...
inline void initState(){
    ...
    const std::string dirCico = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<CicoState>(new CicoState(dev::u256(0), CicoState::openDB(dirCico, hashDB, dev::WithExisting::Trust), dirCico + "/cicoDB", dev::eth::BaseState::Empty));
...
inline dev::Address createCicoAddress(dev::h256 hashTx, uint32_t voutNumber){
...
inline CicoTransaction createCicoTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout = 0){
    CicoTransaction txEth;
    if(recipient == dev::Address()){
        txEth = CicoTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = CicoTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
...
inline std::pair<std::vector<ResultExecute>, ByteCodeExecResult> executeBC(std::vector<CicoTransaction> txs){
