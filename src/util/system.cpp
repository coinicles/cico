...
const char * const BITCOIN_CONF_FILENAME = "cico.conf";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Cico
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Cico
    // Mac: ~/Library/Application Support/Cico
    // Unix: ~/.cico
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Cico";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Cico";
#else
    // Unix
    return pathRet / ".cico";
#endif
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (copyright_devs.find("Cico Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Cico Core Developers";
    }
