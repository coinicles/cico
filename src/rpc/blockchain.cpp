...
UniValue blockheaderToJSON(const CBlockIndex* tip, const CBlockIndex* blockindex)
{
    ...
    result.pushKV("hashStateRoot", blockindex->hashStateRoot.GetHex()); // cico
    result.pushKV("hashUTXORoot", blockindex->hashUTXORoot.GetHex()); // cico

    if(blockindex->IsProofOfStake()){
        result.pushKV("prevoutStakeHash", blockindex->prevoutStake.hash.GetHex()); // cico
        result.pushKV("prevoutStakeVoutN", (int64_t)blockindex->prevoutStake.n); // cico
    }
...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* tip, const CBlockIndex* blockindex, bool txDetails)
{
    ...
    result.pushKV("hashStateRoot", block.hashStateRoot.GetHex()); // cico
    result.pushKV("hashUTXORoot", block.hashUTXORoot.GetHex()); // cico
...
UniValue transactionReceiptToJSON(const CicoTransactionReceipt& txRec)
{
    UniValue result(UniValue::VOBJ);
...
UniValue callcontract(const JSONRPCRequest& request)
{
    ...
    if(request.params.size() >= 3){
        CTxDestination cicoSenderAddress = DecodeDestination(request.params[2].get_str());
        if (IsValidDestination(cicoSenderAddress)) {
            const PKHash *keyid = boost::get<PKHash>(&cicoSenderAddress);
...
UniValue getdelegationinfoforaddress(const JSONRPCRequest& request)
{
            RPCHelpMan{"getdelegationinfoforaddress",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The cico address string"},
                },
                RPCResult{
            "{\n"
    ...
    CicoDelegation cicoDelegation;
    Delegation delegation;
    uint160 address = uint160(*pkhash);
    if(!cicoDelegation.GetDelegation(address, delegation)) {
        throw JSONRPCError(RPC_INTERNAL_ERROR, "Failed to get delegation");
    }
    bool verified = cicoDelegation.VerifyDelegation(address, delegation);
...
UniValue getdelegationsforstaker(const JSONRPCRequest& request)
{
            RPCHelpMan{"getdelegationsforstaker",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The cico address string for staker"},
                },
                RPCResult{
            "[{\n"
    ...
    if(!cicoDelegation.FilterDelegationEvents(events, filter)) {
        throw JSONRPCError(RPC_INTERNAL_ERROR, "Failed to get delegations for staker");
    }
    std::map<uint160, Delegation> delegations = cicoDelegation.DelegationsFromEvents(events);
...
UniValue gettxout(const JSONRPCRequest& request)
{
            ...
            "     \"addresses\" : [          (array of string) array of cico addresses\n"
            "        \"address\"     (string) cico address\n"
            "        ,...\n"
            "     ]\n"
            "  },\n"
