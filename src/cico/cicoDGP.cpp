...
void CicoDGP::initDataSchedule(){
    dataSchedule = scheduleDataForBlockNumber(0);
}

bool CicoDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData, int blockHeight){
...
dev::eth::EVMSchedule CicoDGP::getGasSchedule(int blockHeight){
    clear();
...
uint64_t CicoDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t CicoDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t CicoDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t CicoDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool CicoDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void CicoDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void CicoDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void CicoDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void CicoDGP::createParamsInstance(){
...
dev::Address CicoDGP::getAddressForBlock(unsigned int blockHeight){
...
void CicoDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void CicoDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void CicoDGP::parseStorageOneUint64(uint64_t& value){
...
void CicoDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule CicoDGP::createEVMSchedule(const dev::eth::EVMSchedule &_schedule, int blockHeight){
