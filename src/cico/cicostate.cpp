...
#include <cico/cicostate.h>

using namespace std;
using namespace dev;
using namespace dev::eth;

CicoState::CicoState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = CicoState::openDB(_path + "/cicoDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

CicoState::CicoState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute CicoState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, CicoTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createCicoAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            cico::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
    ...
    if(voutLimit){
        ...
        return ResultExecute{ex, CicoTransactionReceipt(oldStateRoot, oldUTXORoot, gas, e.logs()), refund.vout.empty() ? CTransaction() : CTransaction(refund)};
    }else{
        return ResultExecute{res, CicoTransactionReceipt(rootHash(), rootHashUTXO(), startGasUsed + e.gasUsed(), e.logs()), tx ? *tx : CTransaction()};
    }
}

std::unordered_map<dev::Address, Vin> CicoState::vins() const // temp
{
...
void CicoState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* CicoState::vin(dev::Address const& _a) const
{
    return const_cast<CicoState*>(this)->vin(_a);
}

Vin* CicoState::vin(dev::Address const& _addr)
{
...
// void CicoState::commit(CommitBehaviour _commitBehaviour)
// {
...
void CicoState::kill(dev::Address _addr)
{
...
void CicoState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
void CicoState::deleteAccounts(std::set<dev::Address>& addrs){
...
void CicoState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void CicoState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
    clog(dev::VerbosityWarning, "exec") << "VM exception:" << ss.str();
}

void CicoState::validateTransfersWithChangeLog(){
...
void CicoState::deployDelegationsContract(){
    dev::Address delegationsAddress = uintToh160(Params().GetConsensus().delegationsAddress);
    if(!CicoState::addressInUse(delegationsAddress)){
        CicoState::createContract(delegationsAddress);
        CicoState::setCode(delegationsAddress, bytes{fromHex(DELEGATIONS_CONTRACT_CODE)});
        commit(CommitBehaviour::RemoveEmptyAccounts);
        db().commit();
    }
}
