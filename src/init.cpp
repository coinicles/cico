 ...
 */
static const char* BITCOIN_PID_FILENAME = "cicod.pid";
...
void Shutdown(InitInterfaces& interfaces)
{
    ...
    util::ThreadRename("cico-shutoff");
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/coinicles/cico>";
    const std::string URL_WEBSITE = "<https://coinicles.tech>";
...
void DeleteBlockChainData()
{
    ...
    fs::remove_all(GetDataDir() / "stateCico");
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                fs::path cicoStateDir = GetDataDir() / "stateCico";
                bool fStatus = fs::exists(cicoStateDir);
                const std::string dirCico(cicoStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsCicostate = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<CicoState>(new CicoState(dev::u256(0), CicoState::openDB(dirCico, hashDB, dev::WithExisting::Trust), dirCico, existsCicostate));
                dev::eth::ChainParams cp(chainparams.EVMGenesisInfo());
                globalSealEngine = std::unique_ptr<dev::eth::SealEngineFace>(cp.createSealEngine());

                pstorageresult.reset(new StorageResults(cicoStateDir.string()));
                if (fReset) {
            ...
            try {
                ...
                globalSealEngine->setCicoSchedule(cicoDGP.getGasSchedule(::ChainActive().Height() + (::ChainActive().Height()+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1) ));
