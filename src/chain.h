...
public:
    ...
    uint32_t nBits;
    uint32_t nNonce;
    uint256 hashStateRoot; // cico
    uint256 hashUTXORoot; // cico
    ...
    void SetNull()
    {
        ...
        nBits          = 0;
        nNonce         = 0;
        hashStateRoot  = uint256(); // cico
        hashUTXORoot   = uint256(); // cico
    ...
    explicit CBlockIndex(const CBlockHeader& block)
    {
        ...
        nNonce         = block.nNonce;
        nMoneySupply   = 0;
        hashStateRoot  = block.hashStateRoot; // cico
        hashUTXORoot   = block.hashUTXORoot; // cico
        nStakeModifier = uint256();
        hashProof = uint256(); 
        prevoutStake   = block.prevoutStake; // cico
        vchBlockSigDlgt    = block.vchBlockSigDlgt; // cico
    }
    ...
    CBlockHeader GetBlockHeader() const
    {
        ...
        block.nBits          = nBits;
        block.nNonce         = nNonce;
        block.hashStateRoot  = hashStateRoot; // cico
        block.hashUTXORoot   = hashUTXORoot; // cico
...
public:
    ...
    inline void SerializationOp(Stream& s, Operation ser_action) {
        ...
        READWRITE(nBits);
        READWRITE(nNonce);
        READWRITE(hashStateRoot); // cico
        READWRITE(hashUTXORoot); // cico
        READWRITE(nStakeModifier);
        READWRITE(prevoutStake);
        READWRITE(hashProof);
        READWRITE(vchBlockSigDlgt); // cico
    }
