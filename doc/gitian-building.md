Gitian building
================
*Setup instructions for a Gitian build of Cico Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Cico
...
- [Building Cico Core](#building-cico-core)
...
Clone the git repositories for cico and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/coinicles/cico --recursive
git clone https://github.com/coinicles/gitian.sigs.git
```
...
```bash
sudo cp /home/debian/cico/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the cico repository under 'Fetch and create inputs' to install sources which require
...
To build Cico Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the cico repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/cico/.git/
...
For example:
```bash
URL=https://github.com/laanwj/cico.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit cico=${COMMIT} --url cico=${URL} ../cico/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit cico=${COMMIT} --url cico=${URL} ../cico/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit cico=${COMMIT} --url cico=${URL} ../cico/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-cico:
./bin/gbuild --commit cico=${COMMIT},cpp-eth-cico=${ETHCOMMIT} --url cico=${URL},cpp-eth-cico=${ETHURL} ../cico/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the cico git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../cico/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/coinicles/cico-detached-sigs.git

BTCPATH=/some/root/path/cico
SIGPATH=/some/root/path/cico-detached-sigs

./bin/gbuild --url cico=${BTCPATH},signature=${SIGPATH} ../cico/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/cico-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/cico-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/cico-osx-build.assert
```
...
[coinicles/gitian.sigs](https://github.com/coinicles/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (info@coinicles.tech) and he will commit them.
